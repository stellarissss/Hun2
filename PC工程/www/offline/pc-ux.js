...
(function (global) {
    ...
    var DRAG_THRESHOLD = 4;          // 拖曳判定阈值（CSS 像素）

    /**
     * 校正 hasInput_TouchNotMouse：hun_min.js 末尾写着
     *     hasInput_TouchNotMouse = channel==CHANNEL_PC ? !1 : !0;
     *     hasInput_TouchNotMouse = !0;        // ← 无条件覆盖，PC 判断被抹掉
     * 该全局量决定 gCanvas.onclick 的取值路径：
     *     true  → 直接取 clientX（触摸语义，无抖动过滤）
     *     false → 用 mouseDownX/mouseDownY 做 3px 抖动过滤（鼠标语义）
     * PC 端应走 false 分支，且鼠标按下时原生 userMouse 会填 mouseDownX/Y，
     * 两侧自洽。此处把它纠正回 PC 应有的取值。
     *
     * 注意：这只影响「点击」的抖动过滤，不影响拖曳（拖曳靠 touchMove_StepX）。
     */
    function installInputModeFix() {
        try {
            if (global.hasInput_TouchNotMouse === false) return true;
            global.hasInput_TouchNotMouse = false;
            log('输入模式已切至 PC（点击启用 3px 抖动过滤）');
            return true;
        } catch (e) { return false; }
    }

    /** 周期性确认 hasInput_TouchNotMouse 保持 PC 取值（廉价、幂等） */
    function installInputModeGuard() {
        if (global.__pcInputModeGuard) return true;
        global.__pcInputModeGuard = true;
        setInterval(function () {
            try { if (global.hasInput_TouchNotMouse !== false) global.hasInput_TouchNotMouse = false; }
            catch (e) { }
        }, 1000);
        return true;
    }
    ...
    function installMouseInput() {
        ...
        function isDragging() { return mouseState.dragging; }
        ...
        // ---- click 抑制：拖曳结束后，丢掉紧随其后的那一次 click ----
        // 游戏的点击检测依赖 clickX/clickY（在 gCanvas.onclick 里赋值）。
        // 拖曳松手时浏览器会补发 click，若不拦截会误触城市/热点。
        g.addEventListener('click', function (e) {
            if (isDragging()) {
                e.stopImmediatePropagation();
                e.preventDefault();
                mouseState.dragging = false;        // 只吞掉一次
            }
        ...
        log('鼠标拖曳已启用（原生 userMouse + 拖曳检测 + 点击抑制）');
        return true;
    }
    ...
    function install() {
        ...
        // 轻量守护：部分分支会在运行期重写该标志，定期纠正回来
        installInputModeGuard();
